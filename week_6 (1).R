...
marriageTaoYuanLate |>
  arrange(desc(prop_marry_pair_late)) |>
  pull(site_id2) -> newLevels
newLevels

makeup=Makeup()
makeup$geom_col() -> mk_geom_col
ggplot(data = marriageTaoYuanLate) +
  mk_geom_col$geom(
    mapping = aes(
      x = factor(site_id2, levels = newLevels),
      y = prop_marry_pair_late
    ),
    fill = "dodgerblue3"
  ) +
  mk_geom_col$scale_y_continuous() +
  makeup$theme() +
  coord_flip() +
  theme(
    axis.ticks.x=element_blank(),
    panel.grid.major.y = element_blank(),
    panel.grid.major.x = element_line(color="#c4d1d7"),
